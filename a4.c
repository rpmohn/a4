 ...
 *
 * Hardcoded button-1 behavior in TERM windows (not configurable via INI):
 *   click-1 / C-click-1       — focus on press, release consumed
 *   dbl-click-1 / C-dbl-click-1 — word select on press, release consumed
 *   drag-1 / C-drag-1         — char select; dbl-drag extends by word
 *
 * Ctrl variants in altscreen additionally bypass forwarding to the PTY
 * so the above behavior applies even inside apps like vim. */
static bool mouse_selection(TickitMouseEventInfo *m, MWin *mw) {
	/* For altscreen terminals, forward events to the PTY.
